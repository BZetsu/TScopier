/*
  Temporary: block high-volume spam signup domains entirely.
  hotmail.com, outlook.com, outlook.co.uk, proton.me
*/
...
BEGIN
  ...
  IF domain_part IN (
    ...
    RAISE EXCEPTION 'This email is not allowed.' USING ERRCODE = 'P0001';
  END IF;
  ...
  -- Remaining MS consumer domains still use name+digits bot pattern
  IF domain_part IN (
    'live.com',
    'msn.com',
    'hotmail.fr',
    ...
    'live.fr',
    'hotmail.co.uk'
  ) AND local_part ~ '^[a-z]{3,16}[0-9]{5,}$' THEN
    RAISE EXCEPTION 'This email is not allowed.' USING ERRCODE = 'P0001';
  END IF;
...
COMMENT ON FUNCTION public.block_spam_auth_signup() IS
  'Rejects spam signups; temporarily blocks hotmail.com, outlook.com, outlook.co.uk, proton.me entirely.';
