...
const SLASH_INSTRUMENT_ALIASES: Record<string, string> = {
  GOLD: "XAUUSD",
  XAU: "XAUUSD",
  SILVER: "XAGUSD",
  XAG: "XAGUSD",
...
function slashSideToSymbol(part: string): string | null {
  const upper = String(part ?? "").toUpperCase().trim()
  if (!upper) return null
  if (SLASH_INSTRUMENT_ALIASES[upper]) return SLASH_INSTRUMENT_ALIASES[upper]
...
export function resolveSlashInstrumentPair(leftRaw: string, rightRaw: string): string | null {
  const left = slashSideToSymbol(leftRaw)
  const right = slashSideToSymbol(rightRaw)
  if (left && right) {
    if (left === right) return left
  }
  if (left && SLASH_INSTRUMENT_ALIASES[String(rightRaw).toUpperCase().trim()] === left) return left
  if (right && SLASH_INSTRUMENT_ALIASES[String(leftRaw).toUpperCase().trim()] === right) return right
...
export function sanitizeParsedSymbol(symbol: string | null | undefined): string | null {
  ...
  if (cleaned === "XAUUSDGOLD" || cleaned === "GOLDXAUUSD") return "XAUUSD"
  if (cleaned === "XAGUSDSILVER" || cleaned === "SILVERXAGUSD") return "XAGUSD"
  if (isTradableInstrumentSymbol(cleaned)) return cleaned
