...
## Core concepts
- **Pause / resume copier**: **set_copier_paused** pauses/resumes the ENTIRE copier (all brokers). For a single account (e.g. “stop Exness Demo”), use **set_broker_active** with is_active=false/true — do not pause the whole copier.
...
export const FEATURE_TOPICS: Record<string, string> = {
  ...
  pause_resume:
    'set_copier_paused pauses the whole copier for every broker. To stop only one broker from copying, use set_broker_active (is_active=false). Open trades already at the broker are not closed by pause.',
  billing:
