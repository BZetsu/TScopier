/** Keep in sync with worker/src/signalIntent/coerceTradeIntent.ts */
import type { TradeIntent, TradeIntentKind, TradeIntentPriceUnit, TradeIntentSide } from './tradeIntent.ts'
...
function numList(v: unknown): number[] {
  ...
  return v.map(numOrNull).filter((n): n is number => n != null)
}

function sideFromRaw(v: unknown): TradeIntentSide | null {
...
function unitFromRaw(v: unknown, fallback: TradeIntentPriceUnit): TradeIntentPriceUnit {
  const u = String(v ?? '').trim().toLowerCase()
  return u === 'pips' ? 'pips' : fallback
}

function entryFromRaw(raw: Record<string, unknown>): number[] {
...
export function coerceTradeIntent(raw: unknown): TradeIntent {
  const j = (raw && typeof raw === 'object' ? raw : {}) as Record<string, unknown>
  const flagsRaw = (j.flags && typeof j.flags === 'object' ? j.flags : j) as Record<string, unknown>

  const kind = kindFromRaw(j.kind ?? j.intent)
  let side = sideFromRaw(j.side ?? j.action)
  if (!side) {
    const action = String(j.action ?? '').toLowerCase()
    if (action === 'buy' || action === 'long') side = 'BUY'
    if (action === 'sell' || action === 'short') side = 'SELL'
  }
