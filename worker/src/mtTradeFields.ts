...
export function resolveMtTicket(order: RawMtOrder): number {
  const flat = flattenMtOrder(order)
  const ticket = Number(pickMtField(flat, 'ticket', 'Ticket', 'order', 'Order', 'deal', 'Deal') ?? 0)
...
export function ingestMtHistoryRows(target: Map<string, RawMtOrder>, rows: unknown[]): void {
