...
describe('buildDesiredLegTargets', () => {
  ...
  it('ladders distributed per-leg TPs across naked legs (50/30/20 -> TP1/TP2/TP3)', () => {
    // 4 naked legs (no broker TP). Distribution map mirrors Targets % output.
    const legs = [
      leg({ id: 'l1', metaapi_order_id: '100' }),
      leg({ id: 'l2', metaapi_order_id: '101' }),
      leg({ id: 'l3', metaapi_order_id: '102' }),
      leg({ id: 'l4', metaapi_order_id: '103' }),
    ]
    const t = buildDesiredLegTargets({
      legs,
      snapshot: [
        open(100, { takeProfit: null }),
        open(101, { takeProfit: null }),
        open(102, { takeProfit: null }),
        open(103, { takeProfit: null }),
      ],
      effectiveSl: 4040,
      effectiveTpLevels: [4055, 4060, 4065],
      isBuy: true,
      perLegTakeProfit: [4055, 4055, 4060, 4065],
      allowTpRepaint: true,
    })
    assert.deepEqual(t.map(x => x.takeProfit), [4055, 4055, 4060, 4065], 'each leg gets its distributed TP, not all deepest')
  })

  it('repaints an existing wrong TP when layering (allowTpRepaint=true)', () => {
    // Reproduces the bug: every leg sits on the deepest TP (4065); distribution
    // should move them back to their laddered targets while still layering.
    const legs = [
      leg({ id: 'l1', metaapi_order_id: '100' }),
      leg({ id: 'l2', metaapi_order_id: '101' }),
    ]
    const t = buildDesiredLegTargets({
      legs,
      snapshot: [open(100, { takeProfit: 4065 }), open(101, { takeProfit: 4065 })],
      effectiveSl: 4040,
      effectiveTpLevels: [4055, 4060, 4065],
      isBuy: true,
      perLegTakeProfit: [4055, 4060],
      allowTpRepaint: true,
    })
    assert.deepEqual(t.map(x => x.takeProfit), [4055, 4060], 'repaints deepest-only TPs back to laddered targets')
  })

  it('never repaints a present broker TP when frozen (allowTpRepaint=false)', () => {
    // A TP was hit -> gate frozen. Open legs keep their broker TP; only naked legs fill.
    const legs = [
      leg({ id: 'l1', metaapi_order_id: '100' }),
      leg({ id: 'l2', metaapi_order_id: '101' }),
    ]
    const t = buildDesiredLegTargets({
      legs,
      snapshot: [open(100, { takeProfit: 4055 }), open(101, { takeProfit: null })],
      effectiveSl: 4040,
      effectiveTpLevels: [4055, 4060, 4065],
      isBuy: true,
      perLegTakeProfit: [4060, 4060],
      allowTpRepaint: false,
    })
    const byTicket = new Map(t.map(x => [x.ticket, x.takeProfit]))
    assert.equal(byTicket.get(100), 4055, 'present broker TP preserved (no repaint after a TP hit)')
    assert.equal(byTicket.get(101), 4060, 'naked leg backfilled with its distributed TP')
  })

  it('falls back to deepest TP when no distribution is supplied (backward compatible)', () => {
    const t = buildDesiredLegTargets({
      legs: [leg({ metaapi_order_id: '100' })],
      snapshot: [open(100, { takeProfit: null })],
      effectiveSl: 4040,
    ...
    assert.equal(t[0]!.takeProfit, 4065, 'no perLegTakeProfit -> deepest fallback unchanged')
  })

  it('preserves per-leg breakeven SL even while repainting TPs', () => {
    const legs = [
      leg({ id: 'l1', metaapi_order_id: '100', sl: 4050, auto_be_applied_at: '2026-06-26T13:20:00Z' }),
      leg({ id: 'l2', metaapi_order_id: '101' }),
    ]
    const t = buildDesiredLegTargets({
      legs,
      snapshot: [open(100, { stopLoss: 4050, takeProfit: 4065 }), open(101, { takeProfit: 4065 })],
      effectiveSl: 4040,
      effectiveTpLevels: [4055, 4060, 4065],
      isBuy: true,
      effectiveSource: 'channel_memory',
    ...
    const byTicket = new Map(t.map(x => [x.ticket, x]))
    assert.equal(byTicket.get(100)!.stoploss, 4050, 'breakeven SL kept, not loosened to 4040')
    assert.equal(byTicket.get(100)!.takeProfit, 4055, 'TP still redistributed on the BE leg')
    assert.equal(byTicket.get(101)!.stoploss, 4040, 'non-BE leg uses effective SL')
  })
})
