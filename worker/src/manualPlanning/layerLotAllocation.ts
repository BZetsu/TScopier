export type LayerLotAllocationReason =
  | 'invalid_total_lot'
  | 'invalid_layer_count'
  | 'invalid_min_lot'
  | 'invalid_lot_step'
  | 'total_lot_below_minimum'
  | 'funded_layer_count_reduced'
  | 'allocation_reduced_by_lot_step'

export interface AllocateLayerLotsInput {
...
export interface LayerLotAllocationSuccess {
  ok: true
  requestedLayerCount: number
  fundedLayerCount: number
  lots: readonly number[]
  intendedTotalLot: number
  allocatedTotalLot: number
  unallocatedLot: number
  reasons: readonly LayerLotAllocationReason[]
}

export interface LayerLotAllocationFailure {
  ok: false
  reason: LayerLotAllocationReason
}

export type LayerLotAllocationResult = LayerLotAllocationSuccess | LayerLotAllocationFailure
...
function toLotUnits(value: number, lotStep: number): number {
...
function fromLotUnits(units: number, lotStep: number): number {
...
export function allocateLayerLots(input: AllocateLayerLotsInput): LayerLotAllocationResult {
  ...
  const lots = Array.from({ length: fundedLayerCount }, (_, idx) => (
    fromLotUnits(baseUnits + (idx < remainderUnits ? 1 : 0), lotStep)
  ))
  ...
  const reasons: LayerLotAllocationReason[] = []

  if (fundedLayerCount < layerCount) reasons.push('funded_layer_count_reduced')
  if (unallocatedLot > LOT_EPS) reasons.push('allocation_reduced_by_lot_step')
