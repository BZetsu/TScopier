...
exports.normalizeDerivAlias = normalizeDerivAlias;
exports.isDerivSyntheticSymbol = isDerivSyntheticSymbol;
exports.derivSyntheticFamily = derivSyntheticFamily;
exports.derivDisplayCandidates = derivDisplayCandidates;
exports.resolveDerivCanonicalToBrokerSymbol = resolveDerivCanonicalToBrokerSymbol;
exports.listDerivCanonicalSymbols = listDerivCanonicalSymbols;
 ...
 */
function normalizeDerivAlias(raw) {
    if (raw == null)
        return null;
    ...
    if (/\bRDBULL\b/.test(u) || /\bBULL\s*MARKET\b/.test(u))
        return 'RDBULL';
    if (/\bRDBEAR\b/.test(u) || /\bBEAR\s*MARKET\b/.test(u))
        return 'RDBEAR';
    // Step indices (STPRNG / STPRNG2 / STPRNG5; Step 100/200/500).
    let m = u.match(/\bSTPRNG\s*([25])?\b/);
    if (m)
        return m[1] === '2' ? 'STPRNG2' : m[1] === '5' ? 'STPRNG5' : 'STPRNG';
    m = u.match(/\bSTEP\s*(\d{3})\b/);
    if (m)
        return m[1] === '200' ? 'STPRNG2' : m[1] === '500' ? 'STPRNG5' : 'STPRNG';
    if (/\bSTEP(?:\s*INDEX)?\b/.test(u))
        return 'STPRNG';
    // Range break indices.
    {
        const n = matchNum(u, /\bRB[_\s]?(\d{2,3})\b/) ?? matchNum(u, /\bRANGE\s*BREAK\s*(\d{2,3})\b/);
        if (n != null && RANGE_BREAK_LEVELS.has(n))
            return `RB_${n}`;
    }
    // Boom / Crash.
    {
        const n = matchNum(u, /\bBOOM\s*(\d{3,4})\b/);
        if (n != null && BOOM_CRASH_LEVELS.has(n))
            return `BOOM${n}`;
    }
    {
    ...
    // Jump.
    {
        ...
        if (n != null && JUMP_LEVELS.has(n))
            return `JD${n}`;
    }
    // Volatility (the most common, and most alias-heavy family).
    const volNum = matchNum(u, /\b1HZ(\d{1,3})V\b/)
...
const CANONICAL_RE = /^(R_\d{1,3}|1HZ\d{1,3}V|BOOM\d{3,4}|CRASH\d{3,4}|STPRNG[25]?|JD\d{2,3}|RB_\d{2,3}|RDBULL|RDBEAR)$/;
/** True when the symbol is already a canonical Deriv synthetic code. */
function isDerivSyntheticSymbol(symbol) {
    return CANONICAL_RE.test(String(symbol ?? '').toUpperCase().trim());
}
/** Family of a canonical Deriv code, or null when not a synthetic. */
function derivSyntheticFamily(canonical) {
    const s = String(canonical ?? '').toUpperCase().trim();
    if (/^R_\d{1,3}$/.test(s) || /^1HZ\d{1,3}V$/.test(s))
        return 'volatility';
    if (/^BOOM\d{3,4}$/.test(s))
        return 'boom';
    if (/^CRASH\d{3,4}$/.test(s))
        return 'crash';
    if (/^STPRNG[25]?$/.test(s))
        return 'step';
    if (/^JD\d{2,3}$/.test(s))
 ...
 */
function derivDisplayCandidates(canonical) {
    const s = String(canonical ?? '').toUpperCase().trim();
    ...
    if (fam === 'step') {
        if (s === 'STPRNG2')
            return [s, 'Step 200 Index', 'Step 200'];
        if (s === 'STPRNG5')
            return [s, 'Step 500 Index', 'Step 500'];
    ...
    // bull_bear
    if (s === 'RDBULL')
        return [s, 'Bull Market Index', 'Bull Market'];
 ...
 */
function resolveDerivCanonicalToBrokerSymbol(canonical, inventory) {
    if (!isDerivSyntheticSymbol(canonical) || !inventory || inventory.length === 0)
        return null;
    ...
    for (const inv of inventory) {
        if (wanted.has(normForMatch(inv)))
            return inv;
    }
    ...
    for (const inv of inventory) {
        ...
        for (const w of sorted) {
            if (w.length >= 4 && (ni === w || ni.startsWith(w) || ni.includes(w)))
                return inv;
        }
    }
    return null;
...
function listDerivCanonicalSymbols() {
    const out = [];
    ...
    for (const n of JUMP_LEVELS)
        out.push(`JD${n}`);
    for (const n of RANGE_BREAK_LEVELS)
        out.push(`RB_${n}`);
