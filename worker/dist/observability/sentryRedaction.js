...
exports.redactStringForSentry = redactStringForSentry;
exports.safeForSentry = safeForSentry;
exports.normalizedErrorCode = normalizedErrorCode;
const MAX_DEPTH = 6;
...
function stripUrlSecrets(input) {
    try {
...
function redactStringForSentry(value) {
...
function safeError(err, opts) {
    ...
    if (err.stack)
        out.stack = redactStringForSentry(err.stack);
    const cause = err.cause;
    if (cause !== undefined)
        out.cause = safeForSentry(cause, { ...opts, seen, depth: (opts.depth ?? 0) + 1 });
    const aggregateErrorCtor = globalThis.AggregateError;
    if (aggregateErrorCtor && err instanceof aggregateErrorCtor) {
        out.errors = safeForSentry(err.errors, { ...opts, seen, depth: (opts.depth ?? 0) + 1 });
    }
    for (const [key, value] of safeEntries(err)) {
...
function safeForSentry(input, opts = {}) {
    ...
    if (input instanceof Error)
        return safeError(input, { ...opts, seen: opts.seen ?? new WeakSet() });
    if (input instanceof URL)
        return redactStringForSentry(input.toString());
    if (input instanceof Date)
        return Number.isFinite(input.getTime()) ? input.toISOString() : '[Invalid Date]';
    const seen = opts.seen ?? new WeakSet();
    const obj = input;
    if (seen.has(obj))
        return '[Circular]';
    ...
    if (Array.isArray(input)) {
        const items = input.slice(0, MAX_ARRAY_ITEMS).map(item => safeForSentry(item, { seen, depth: depth + 1 }));
        if (input.length > MAX_ARRAY_ITEMS)
            items.push(`[TRUNCATED ${input.length - MAX_ARRAY_ITEMS} items]`);
        return items;
    }
    if (input instanceof Map) {
        ...
        if (input.size > MAX_ARRAY_ITEMS)
            items.push(['__truncated_items', input.size - MAX_ARRAY_ITEMS]);
        return { type: 'Map', entries: items };
    }
    if (input instanceof Set) {
        const items = Array.from(input.values()).slice(0, MAX_ARRAY_ITEMS).map(item => safeForSentry(item, { seen, depth: depth + 1 }));
        if (input.size > MAX_ARRAY_ITEMS)
            items.push(`[TRUNCATED ${input.size - MAX_ARRAY_ITEMS} items]`);
    ...
    if (totalKeys > MAX_OBJECT_KEYS)
        out.__truncated_keys = totalKeys - MAX_OBJECT_KEYS;
    return out;
}
function normalizedErrorCode(err, fallback = 'UNKNOWN') {
