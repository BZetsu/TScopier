...
exports.brokerPendingOperationForLayer = brokerPendingOperationForLayer;
exports.buildLayeringBrokerPendingClientReference = buildLayeringBrokerPendingClientReference;
exports.findBrokerOrderByClientReference = findBrokerOrderByClientReference;
exports.brokerOrderMatchesLayer = brokerOrderMatchesLayer;
exports.validateBrokerPendingPrice = validateBrokerPendingPrice;
exports.reconcileNativePendingLegByReference = reconcileNativePendingLegByReference;
exports.activateLayeringBrokerPendingOrders = activateLayeringBrokerPendingOrders;
...
const layeringBrokerCapability_1 = require("../layeringBrokerCapability");
...
function brokerPendingOperationForLayer(side) {
    return side === 'buy' ? 'BuyLimit' : 'SellLimit';
}
function buildLayeringBrokerPendingClientReference(args) {
...
function brokerOrderMatchesLayer(args) {
    ...
    const round = (n) => Number(n.toFixed(args.digits));
...
function validateBrokerPendingPrice(args) {
    if (!Number.isFinite(args.price) || args.price <= 0)
        return 'broker_pending_invalid_price';
...
async function reconcileByReference(prep, reference) {
    if (!prep.api)
        return null;
...
async function placeOrAdoptPending(args) {
    ...
    if (!q)
        return { ok: false, reason: 'broker_pending_quote_unavailable' };
    ...
    if (priceReason)
        return { ok: false, reason: priceReason };
    const roundedLot = (0, helpers_1.roundLot)(row.volume, prep.params);
...
const NATIVE_FIRST_SEND_STATUSES = ['planned'];
const NATIVE_RECONCILIATION_STATUSES = ['submission_claimed', 'submission_ambiguous', 'reconciliation_required', 'submitted'];
function claimOwner() {
    return `layering-native:${process.pid}:${Date.now()}:${Math.random().toString(36).slice(2)}`;
}
function legSelectColumns() {
    return [
        'id',
        'layer_plan_id',
        'layer_plan_metadata',
...
async function loadNativePendingRows(prep, snapshot) {
...
async function markNativeLegBlocked(prep, row, reason) {
...
async function reconcileNativePendingLegByReference(args) {
    ...
    if (!NATIVE_RECONCILIATION_STATUSES.includes(status)) {
    ...
    if (!brokerOrderMatchesLayer({
        ...
        await (0, layeringPlanLifecycle_1.markLayeringPlanInvalid)(args.prep.ctx.supabase, args.snapshot.planId, 'broker_pending_conflict');
...
async function planStillActive(prep, snapshot) {
    const { data, error } = await prep.ctx.supabase
        .from('layering_plans')
    ...
    if (String(row.status ?? '') !== 'active')
        return false;
    const parsed = (0, layeringPlanPersistence_1.parsePersistedLayeringPlan)(row.layer_plan_metadata);
    if (!parsed.ok)
        return false;
    const expectedFingerprint = (0, layeringPlanPersistence_1.computeLayeringPlanFingerprint)(snapshot);
    const persistedFingerprint = (0, layeringPlanPersistence_1.computeLayeringPlanFingerprint)(parsed.snapshot);
...
async function placeOrAdoptClaimedPending(args) {
    ...
    const preSendDecision = (0, layeringModeRollout_1.resolveLayeringModeRolloutDecision)({ mode: args.snapshot.mode, brokerAccountId: prep.broker.id });
    ...
    if (existing) {
        if (!brokerOrderMatchesLayer({
            ...
            await (0, layeringPlanLifecycle_1.markLayeringPlanInvalid)(prep.ctx.supabase, args.snapshot.planId, 'broker_pending_conflict');
            return { ok: false, reason: 'broker_pending_conflict' };
        }
        const ticket = ticketFromOrder(existing);
        if (ticket == null || ticket <= 0)
    ...
    if (!placed.ok) {
        ...
        if (placed.reason === 'broker_pending_conflict') {
            await (0, layeringPlanLifecycle_1.markLayeringPlanInvalid)(prep.ctx.supabase, args.snapshot.planId, placed.reason);
        }
        return placed;
    }
    const confirmed = await confirmNativeLeg({
...
async function activateLayeringBrokerPendingOrders(args) {
    const { prep, snapshot } = args;
    if (!prep.api)
        return { ok: false, reason: 'broker_pending_unsupported' };
    if (prep.manual.range_layering_type !== 'pending_order')
        return { ok: false, reason: 'broker_pending_blocked' };
    const capability = (0, layeringBrokerCapability_1.resolveNativePendingCapability)({
        broker: prep.broker,
        api: prep.api,
    });
    if (!capability.supported)
        return { ok: false, reason: 'broker_pending_unsupported' };
    const decision = (0, layeringModeRollout_1.resolveLayeringModeRolloutDecision)({ mode: snapshot.mode, brokerAccountId: prep.broker.id });
    ...
    const placedTickets = [];
    let adopted = 0;
    const activated = await (0, layeringPlanPersistence_1.activateLayeringPlanWithLegs)(prep.ctx.supabase, snapshot, {
    ...
    if (!persistedRows)
        return { ok: false, reason: 'broker_pending_activation_failed' };
    for (const row of persistedRows) {
        const loopDecision = (0, layeringModeRollout_1.resolveLayeringModeRolloutDecision)({ mode: snapshot.mode, brokerAccountId: prep.broker.id });
        ...
        if (row.native_submission_status === 'confirmed' && row.ticket)
            continue;
        if (NATIVE_RECONCILIATION_STATUSES.includes(String(row.native_submission_status ?? ''))) {
        ...
        if (!claimed)
            return { ok: false, reason: 'broker_pending_claim_lost' };
        ...
        if (placed.adopted)
            ...
            placedTickets.push(placed.ticket);
    }
