...
exports.runLayeringModeRangeEntry = runLayeringModeRangeEntry;
const layeringModeCalculators_1 = require("../manualPlanning/layeringModeCalculators");
const layerLotAllocation_1 = require("../manualPlanning/layerLotAllocation");
const layerSizingConstraints_1 = require("../manualPlanning/layerSizingConstraints");
...
function sanitizedLog(event, payload) {
    console.log(JSON.stringify({ event, component: 'layeringModes', ...payload }));
}
function rangeFromParsed(prep) {
...
function clonePrepForSingleLayer(prep, lot, runtime) {
...
async function logLayeringExecutionBlocked(supabase, prep, reason, mode) {
...
function buildSnapshot(args) {
    ...
    const planId = (0, layeringPlanPersistence_1.generateLayerPlanId)(identity);
    if (!planId)
        return { ok: false, reason: 'invalid_plan_id' };
    const built = (0, layeringPlanPersistence_1.buildLayeringPlanSnapshot)({
        ...identity,
        planId,
...
async function persistPreparedPlan(prep, snapshot) {
    const persisted = await (0, layeringPlanPersistence_1.persistLayeringPlan)(prep.ctx.supabase, snapshot);
    if (persisted.ok) {
    ...
    if (persisted.reason === 'terminal_plan')
        return 'terminal_plan';
    return 'persistence_failed';
}
async function activatePreparedPlan(args) {
    const mode = args.snapshot.mode;
    const decision = (0, layeringModeRollout_1.resolveLayeringModeRolloutDecision)({ mode, brokerAccountId: args.prep.broker.id });
    ...
    if (args.prep.manual.range_layering_type === 'pending_order') {
        const activated = await (0, layeringModeBrokerPending_1.activateLayeringBrokerPendingOrders)(args);
    ...
    const expiresAt = Number.isFinite(pendHours) && pendHours > 0
        ...
        : null;
    const rows = (0, layeringPlanPersistence_1.materializeExecutableLayerPlanLegRows)({
    ...
    const activated = await (0, layeringPlanPersistence_1.activateLayeringPlanWithLegs)(args.prep.ctx.supabase, args.snapshot, {
...
function calculatedDynamicFromFill(args) {
    if (args.actualFirstLot <= 0 || args.actualFirstLot > args.intendedTotalLot)
        return null;
    ...
    if (remainingPrices.length > 0 && remainingTotal > 0) {
        const alloc = (0, layerLotAllocation_1.allocateLayerLots)({
    ...
    if (allocatedTotalLot > args.intendedTotalLot)
        return null;
...
async function runLayeringModeRangeEntry(prep) {
    const mode = (0, layeringModes_1.resolveLayeringMode)(prep.manual);
    if (mode !== 'static' && mode !== 'dynamic') {
        return (0, entryExecution_1.finishEntrySend)(prep, false, false, true, false);
    }
    const rollout = (0, layeringModeRollout_1.resolveLayeringModeRolloutDecision)({ mode, brokerAccountId: prep.broker.id });
    ...
    if (mode === 'static') {
        const calculated = (0, layeringModeCalculators_1.calculateStaticLayerPlan)({
            side,
            rangeLow: range.low,
        ...
        if (!calculated.ok)
            return { openedOrMerged: false, finalizeSkipReason: `layering_mode_static_${calculated.reason}` };
        const built = buildSnapshot({ prep, calculatedPlan: calculated, mode, anchorSource: 'signal' });
        if (!built.ok)
            return { openedOrMerged: false, finalizeSkipReason: `layering_mode_static_${built.reason}` };
        ...
        if (rollout.reason === 'prepare_only')
            return { openedOrMerged: true, finalizeSkipReason: 'layering_mode_static_prepare_only' };
        const runtime = {
            mode,
            rollout,
        ...
        return (0, entryExecution_1.finishEntrySend)(nextPrep, false, false, true, false);
    }
    if (rollout.reason === 'prepare_only') {
    ...
    if (firstSizing && !firstSizing.ok)
        return { openedOrMerged: false, finalizeSkipReason: `layering_mode_dynamic_${firstSizing.reason}` };
    const firstAlloc = firstSizing ?? (0, layerLotAllocation_1.allocateLayerLots)({ intendedTotalLot, layerCount: maxLayers, minLot, lotStep });
    if (!firstAlloc.ok)
        return { openedOrMerged: false, finalizeSkipReason: `layering_mode_dynamic_${firstAlloc.reason}` };
    ...
    const runtime = {
        ...
        onImmediateFill: async (fill) => {
            ...
            const prices = (0, layeringModeCalculators_1.calculateDynamicLayerPrices)({
                side,
                rangeLow: range.low,
                rangeHigh: range.high,
