...
exports.normalizeSingleTpTarget = normalizeSingleTpTarget;
exports.resolveSingleTpTargetIndex = resolveSingleTpTargetIndex;
exports.planSinglePartialTps = planSinglePartialTps;
function normalizeSingleTpTarget(raw) {
    const v = String(raw ?? 'farthest').toLowerCase();
    if (v === 'tp1' || v === 'tp2' || v === 'tp3')
        return v;
    return 'farthest';
}
function resolveSingleTpTargetIndex(args) {
    ...
    if (target === 'tp1')
        return 0;
    if (target === 'tp2')
        return Math.min(1, finalTps.length - 1);
    if (target === 'tp3')
        return Math.min(2, finalTps.length - 1);
    if (isBuy === true) {
        let idx = 0;
        for (let i = 1; i < finalTps.length; i++) {
            if ((finalTps[i] ?? 0) > (finalTps[idx] ?? 0))
                idx = i;
        }
        return idx;
    }
