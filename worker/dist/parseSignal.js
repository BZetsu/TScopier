...
function resolveTradeSideFromMessage(message, channelKeywords, lexicon = null) {
    ...
    const instrumentSide = text.match(/\b(?:sell|short)\s+(?:xauusd|xagusd|gold|btcusd|eurusd|[a-z]{6})\b|\b(?:xauusd|xagusd|gold|btcusd|eurusd|[a-z]{6})\s+(?:sell|short)\b/i);
    const instrumentBuy = text.match(/\b(?:buy|long)\s+(?:xauusd|xagusd|gold|btcusd|eurusd|[a-z]{6})\b|\b(?:xauusd|xagusd|gold|btcusd|eurusd|[a-z]{6})\s+(?:buy|long)\b/i);
    if (instrumentSide && !instrumentBuy)
        return 'sell';
    if (instrumentBuy && !instrumentSide)
        return 'buy';
    const sideText = text
