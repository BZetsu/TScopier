...
exports.normalizeFxsocketMtStatus = normalizeFxsocketMtStatus;
exports.isFxsocketMtStatusHealthy = isFxsocketMtStatusHealthy;
exports.listFxsocketMtStatusChecks = listFxsocketMtStatusChecks;
exports.terminalHealthRowPatchFromMtStatus = terminalHealthRowPatchFromMtStatus;
exports.legacyTerminalStatusFromMtStatus = legacyTerminalStatusFromMtStatus;
function num(v) {
...
function normalizeFxsocketMtStatus(raw) {
    ...
    return {
        ...
        account: {
            ...
            tradeAllowed: boolField(account.tradeAllowed ?? account.TradeAllowed ?? account.trade_allowed),
        },
        bridge: {
            version: bridge.version != null ? String(bridge.version) : undefined,
            tradeEaReady: boolField(bridge.tradeEaReady ?? bridge.TradeEaReady ?? bridge.trade_ea_ready),
            symbolsSynced: boolField(bridge.symbolsSynced ?? bridge.SymbolsSynced ?? bridge.symbols_synced),
        },
    };
}
function isFxsocketMtStatusHealthy(status) {
    return listFxsocketMtStatusChecks(status).every(check => check.ok);
}
function listFxsocketMtStatusChecks(status) {
    const statusValue = (status.status ?? '').trim().toLowerCase();
    return [
        { id: 'statusReady', ok: statusValue === 'ready', value: status.status ?? null },
        { id: 'terminalAlive', ok: status.terminal?.alive === true, value: status.terminal?.alive ?? null },
        { id: 'brokerConnected', ok: status.broker?.connected === true, value: status.broker?.connected ?? null },
        { id: 'accountLoggedIn', ok: status.account?.loggedIn === true, value: status.account?.loggedIn ?? null },
        {
...
function terminalHealthRowPatchFromMtStatus(status) {
...
function legacyTerminalStatusFromMtStatus(status) {
