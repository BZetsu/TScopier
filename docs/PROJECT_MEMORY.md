...
- **Follow-ups:** (1) confirm `layeringModeCapabilities.test.ts` failure is a real regression or stale test; (2) run the offline `!atk all` battery against staging and add evasive variants (homoglyph/split-word/emoji encodings) to the corpus; (3) optional LLM-as-judge red-team layer for semantic evasions the regex guard misses; (4) note: `node_modules` was stale/missing `@marsidev/react-turnstile` — run `npm install` after clone.
