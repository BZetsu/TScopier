...
- **What this file is:** a running, chronological changelog of everything done in this project — what changed, why, and what's still left to do. Read this section first; the changelog below has the technical detail.
- **Latest work (2026-08-12):**
  1. **Duplicate-signal fix (biggest).** One Telegram message could be copied twice (e.g. a "close" signal), so trades were closed twice — the second attempt failed with "unknown ticket" and flagged the dashboard red. The same race could also open positions twice. Now the listener claims a message up-front and drops duplicates, the loser is stored as a non-executable "skipped" stub in the DB, and parsing retries on failure so no signal is lost.
  2. **Partial-profit math fix.** "Book 50%" on a tiny position rounded **up** to 100% and closed everything. Partial volumes are now floored (rounded down) and below-minimum sizes are skipped safely.
  3. **Calmer errors.** MT4 error "4108 / Invalid request" is now treated as a harmless "position already gone" for close-type actions, so it no longer shows as a red alert.
  4. **Parsing reliability.** Failed parses (AI timeouts / rate-limits / transport) retry the same signal instead of dropping it.
  5. **Assistant guard.** The in-app AI assistant now screens messages for prompt-injection and strips hidden / zero-width text before anything reaches the model.
  6. **Cerebras key pool.** API keys rotate across several accounts so one account's daily quota can't stall parsing for everyone.
- **Where things stand:** all of the above is **committed, pushed, and merged into the `dev` and `staging` branches**. Still pending: redeploy the listener / trade workers on Railway (staging first, then prod), verify no duplicate management rows appear and stubs land as `skipped`, then promote to `main`.
- **How to read status labels:** "in working tree" = written but not committed; "committed" = saved to git; "on dev / staging / main" = merged into that branch; "deployed" = live on Railway / Supabase. Historical prod data (rows already written) is never rewritten.
...
- **Wiring:**
  1. Every incoming user message is guard-scanned **BEFORE the chat loop**; a flagged turn is refused with HTTP 403 + generic message ("I can't help with that request.") — the matched reason is logged server-side only (`[assistant-chat] turn refused user=… reason=…`), never returned (design: don't teach attackers how to evade).
  2. Approved user content is replaced with the **SANITIZED text** (control/zero-width stripped, capped) before reaching the model — both plain-text and image-caption paths.
  3. Model tool-call args pass through `sanitizeToolArgs` before `executeTool` (bounds depth/key-count/lengths, drops non-JSON-safe values).
  4. The client-confirmed `execute` path args are sanitized the same way before the mutation runs.
...
### 2026-08-12 — Duplicate-dispatch double-close → `unknown ticket` (trade #2348668186) — fix batch committed + merged to dev/staging + parse-retry added
...
- **Root cause (verified via prod DB + Railway logs):**
  1. The provider's close message (tg msg 19742) was processed **twice** by the listener ~960ms apart (invocations at 07:52:09.030Z → signal `94f52e2f`, 07:52:09.990Z → `4ddcf865`).
  2. Both deliveries passed the DB count dedupe (neither persisted yet) AND the in-memory `liveMessageDedup` (set only AFTER parse) — so `4ddcf865` lost the unique-key race → `ensureSignalRow` stub (telegram_message_id NULL, status parsed) → **still dispatched + executed**.
  3. Both close signals ran `mgmt_close` on the same 12 tickets: first wave OK, second wave → broker `unknown ticket` → 12 `order_close_audit failed` rows.
  - The Aug-10/11 benign fixes worked (trades closed, no retry storm) — this was **duplicate execution of the same instruction**, not a loop. 116+ stub signals observed across all users in ~1.5h → same race duplicates ENTRY signals too (double positions, no error surfaces).
...
- **Fix (worker only — now COMMITTED, pushed, and merged into `upstream/dev` + `upstream/staging`):**
  - **Duplicate-dispatch batch** (from the same-day partial_profit 4108 investigation, previously uncommitted):
    1. `userListener.ts` claims message in-memory BEFORE parse (dropped on duplicate), releases claim on parse failure.
    2. `ensureSignalRow.ts` detects the unique-key owner → persists a **non-executable stub** (`status:'skipped'`, `skip_reason:'duplicate_telegram_message'`) and returns `{duplicate, existingSignalId}`.
    3. `dispatch.ts` `handleSignal` returns early on `ensured.duplicate` (worker-side defense).
    4. `managementExecutor.ts` partial volumes FLOOR + below-min-lot skip.
    5. `orderModifyBenign.ts` `isPositionGoneCloseError` (MT4 4108 benign for close-family only).
    6. `partialTpMonitor.ts` 4108 benign.
...
- **Follow-ups (deployer):** committed as `d70c5471` (duplicate-dispatch + parse-retry) on `feature/fix-duplicate-dispatch-4108`, pushed, merged to `upstream/dev` + `upstream/staging` (merge `d9c4e2f8`) — next: Railway listener + trade worker redeploy (let old listener instance fully stop first) → verify no duplicate mgmt rows + stubs land `skipped` → main. Historical prod rows stay as-is. PR #102 (deferred-only range plan) already on staging/dev; Emma's work (PR #94 etc.) verified present on all branches.
...
- **Fixes (worker only, now COMMITTED as `20e6bfc0` on `feature/fix-duplicate-dispatch-4108`, pushed, merged to `upstream/dev` + `upstream/staging`):**
...
- **Follow-ups (deployer):** committed + pushed + merged to dev/staging — next: Railway worker redeploy (staging) → main after validation. Historical prod rows stay as-is (backfill optional). The duplicate-delivery source (live event + poll overlap) is now harmless: worst case a 2nd signal is created but never dispatched.
...
- **Fix 2 (now COMMITTED as `4ef2e234`, merged to dev/staging/main):** `worker/src/autoManagementMonitor.ts` throttles failed `auto_be` log rows to 1 per trade per `FAILURE_LOG_THROTTLE_MS` (default 5 min, env `AUTO_BE_FAILURE_LOG_THROTTLE_MS`). Success/benign paths clear the cooldown. Typecheck clean; `autoManagement.test.ts` 14/14 pass; only pre-existing lint errors (bid/ask no-useless-assignment) untouched.
...
- **Follow-ups:** Fix 2 committed (`4ef2e234`); (2) apply Fix 1 migration to PROD (pending user go-ahead); (3) register the migration row with a proper `text[]` literal; (4) separately investigate the broker `fcabb782` disconnection root cause.
...
### 2026-08-11 — `mgmt_breakeven` unknown-ticket retry loop root-caused + fixed (trade #1297061, signal e8831c0f) — committed + merged to dev/staging/main
...
- **Fix (worker only — now COMMITTED as `00674674` on `feature/fix-unknown-ticket-mgmt`, merged to dev/staging/main):**
...
- **Follow-ups:** committed (`00674674`), pushed, merged to dev/staging/main — Notion task update; consider the empty-snapshot reconcile deferral so flat accounts don't keep zombie `open` rows.
...
- **Follow-ups:** deployed via PR #96 (worker fix on main/staging) — still open: revisit the empty-snapshot reconcile deferral so a flat-but-connected account is not left with permanently-open DB rows.
