...
### 2026-08-14 — Assistant can now surface trades, copier logs & trade reports (get_recent_trades / get_copier_logs / get_trade_detail / report_trade / open_trades)

- **Plain English:** The in-app assistant can now actually see your trades. Ask "what happened with my trades?", "show my copier logs", or "why did this trade fail" and it pulls the real outcome from the database — executed with tickets, skipped with a reason, or failed with the error message — and renders a trades card right in the chat. It can also guide (or file for you, with a Confirm button) a trade report into the support queue, and open the live Trades page.
- **Context (user request):** the assistant previously had no visibility into the user's trades/execution, so trade questions were guessed or deflected. User approved in-chat `report_trade` (Confirm card + manual steps), trade data from `signals` + `trade_execution_logs` only (no live broker positions via `fxsocket-broker` yet), `get_trade_detail` deep-dive, and a visible trades card rendered from `tool_results`.
- **Edge function (`supabase/functions/assistant-chat/index.ts`):**
  - New tools: `get_recent_trades` (top-level signals, parent+legs aggregated, tickets from `response_payload.ticket`, errors from failed execution logs), `get_copier_logs` (status-filterable, mirrors /copier-logs), `get_trade_detail` (single trade + child legs + dispatch claims + execution rows; resolves by `signal_id` OR broker ticket), `report_trade` (confirm-first INSERT into `trade_reports`, pre-fills symbol/direction/ticket/broker/entry/SL/TP/lot from the signal when `signal_id` given), `open_trades` (client action → navigate `/account-trades`).
  - `NAV_ALLOWLIST` += `/account-trades`, `/copier-logs`, `/activities`, `/manage-signals`. `report_trade` added to `EXECUTABLE_MUTATIONS`. `runClientActionTool` `open_trades` case.
  - System prompt rules: call the read tools before answering trade questions; never invent tickets/fills/errors; report flow = identify trade → `report_trade` with category+reason OR manual steps (open_trades → open trade → Report → category → submit); categories `wrong_entry|wrong_sl|wrong_tp|wrong_direction|wrong_lots|not_executed|other`.
  - `assistantKnowledge.ts`: product overview now lists Trades `/account-trades`, Copier Logs `/copier-logs`, Activities `/activities`; FEATURE_TOPICS += `trades`, `copier_logs`, `trade_reports`.
- **Frontend:** `src/lib/assistantActions.ts` allowlist mirrored (4 new paths). New `src/components/assistant/AssistantTradesCard.tsx` renders `tool_results` trades (symbol, buy/sell badge, status badge, time/channel, entry/SL/TP/lot, tickets, skip reason, errors, leg count) for the 3 read tools; wired into `AssistantPanel.tsx` via a `toolResults` state set after each chat/execute response and cleared on next send / clear-conversation. i18n `assistant.tradesCard` added to `en/es/fr` + all 6 `chrome/*` locales + `types.ts`.
- **Tests/verification:** `npx tsc -b` clean; eslint clean on all changed files; frontend `npm test` 318/318 pass. Deployed to staging (`axdcledcyhyvzrnfkwat`) + prod (`sxkpcovbyaficvtkpsdo`). Harness (`/tmp/opencode/verify-trade-tools.mjs` + `verify-report-chat*.mjs`): chat triggers `get_recent_trades`/`get_copier_logs` (trades arrays), read tools NOT directly executable (confirm path only), `report_trade` invalid category rejected, seeded staging signal → `get_trade_detail` returned full detail → chat report flow returned a Confirm card → confirm executed INSERT with `ok:true`. Seeded test signal + test `trade_reports` row cleaned up afterward.
- **Follow-ups:** (1) Netlify rebuild for the frontend card/i18n; (2) consider exposing live broker positions (via `fxsocket-broker`) as a follow-up capability; (3) `scripts/assistant-redteam.mjs` could get a `!trades` quick-check command.

### 2026-08-14 — Assistant red-team: refusal-loop DoS fix + image-caption guard gap + clear-conversation button + test harness

- **Plain English:** while probing the in-app assistant with prompt-injection payloads, one attack message **bricked the assistant for the whole session** — every later message, even a plain "Hello", got refused. Root cause: the prompt-injection guard re-scanned the entire conversation history on every turn, so the old attack text kept triggering refusals forever. Also found: an injection written as an **image caption** (typed next to a screenshot) completely bypassed the guard, and there was **no way to clear the conversation** in the UI to recover.
- **Root cause (technical):** `supabase/functions/assistant-chat/index.ts` ran `guardAssistantUserMessage` over **every** user message in the window (`messages.slice(-20)`) and refused the turn if *any* matched — so a single flagged message poisoned every later turn (client re-sends full history each call). Additionally the guard loop did `typeof msg.content !== "string" → continue`, silently skipping image messages whose content is a `ContentPart[]` array (caption + image parts).
- **Fix 1 — refuse only the newest turn; drop poisoned history (`assistant-chat/index.ts`):** only the **newest** user message may refuse the turn; older flagged messages are treated as untrusted content and **removed from the window** (logged `dropped poisoned history message`) instead of bricking the session. Verified via tsx simulation: poisoned-history + clean message → passes through; fresh attack → still refused; benign "ignore stop loss…" control → still allowed.
- **Fix 2 — guard image captions (`assistant-chat/index.ts`):** the guard now extracts and scans the text parts of array-content (image) messages too; flagged caption on the newest turn → refused, older → dropped. Text parts are still `sanitizeAssistantText`-stripped on the allowed path. (Text *inside* the image pixels remains LLM-defended only — no OCR in the guard.)
- **Fix 3 — clear-conversation button (`src/components/assistant/AssistantPanel.tsx` + i18n):** new `RefreshCw` button in the assistant header clears messages, pending confirmations/actions, and resets telegram/broker flows. New `assistant.clearConversation` string across `en/es/fr` + all `chrome/*` locales.
- **Red-team harness (new tooling):** `scripts/assistant-redteam.mjs` (interactive CLI — REPL chat with the deployed `assistant-chat` + `--create-user` to provision a throwaway staging account for UI testing) and `scripts/assistant-attack-corpus.json` (61 payloads across 11 categories with expect/refuse-allow scoring). Run `node scripts/assistant-redteam.mjs` then `!atk all`.
- **Tests/verification:** `npx tsc -b` clean (after `npm install` restored missing `@marsidev/react-turnstile`); eslint clean on all changed files; frontend `npm test` 317/318 pass — the single failure is pre-existing `src/lib/layeringModeCapabilities.test.ts:15` (expects `fail closed` for static/dynamic auto) and is unrelated to this work.
- **Deploy state:** staging. Requires `supabase functions deploy assistant-chat --project-ref axdcledcyhyvzrnfkwat` + Netlify rebuild for the frontend button/i18n.
- **DEPLOY-MISS INCIDENT (2026-08-14, same day):** the fix was committed but **not deployed** — the deployed `assistant-chat` edge function kept running the pre-fix code, so the user's UI test showed the identical permanent-refusal loop again (base64 payload → every later message including "Hello"/"Yooo" refused). Lesson: a repo-only fix is not a user-facing fix; the edge function is deployed manually and needs a `supabase functions deploy` step. After deploying (`supabase functions deploy assistant-chat --project-ref axdcledcyhyvzrnfkwat --use-api`) the stuck-session replay was verified against the deployed function via `node scripts/assistant-redteam.mjs` login: history with a flagged base64 message + benign `Hello` now answers instead of refusing (dropped poisoned message). Old poisoned sessions self-recover server-side (flagged messages are spliced each request) — no client reset needed.
- **Follow-ups:** (1) ~~confirm~~ `layeringModeCapabilities.test.ts` failure was fixed (`ef08e376`, fail-closed mechanism gating) — 318/318 pass; (2) deploy `assistant-chat` to **prod** (`--project-ref sxkpcovbyaficvtkpsdo`) + Netlify rebuild; (3) run the offline `!atk all` battery against staging and add evasive variants (homoglyph/split-word/emoji encodings) to the corpus; (4) optional LLM-as-judge red-team layer for semantic evasions the regex guard misses; (5) note: `node_modules` was stale/missing `@marsidev/react-turnstile` — run `npm install` after clone.
...
- **Files:** `src/pages/auth/VerifyEmailPage.tsx`, `supabase/functions/send-verification-email/index.ts`.
- **Deploy note:** `supabase functions deploy send-verification-email` + Netlify rebuild for the frontend change.
...
- **Verification:** eslint clean on all 3 files; `node --experimental-strip-types --check` syntax OK; guard logic re-verified via node/tsx (refuse, sanitize, toolArgs bounds). Deno not installed locally — `deno test` for `assistantGuard.test.ts` still pending on a machine with Deno (tests are pure TS, verified equivalent via node).
- **Deploy note:** requires `supabase functions deploy assistant-chat` (and the `_shared` lib ships with it) on staging then prod. No migration, no env vars.
...
- **Fix (worker only — in working tree, NOT yet committed/pushed/deployed):**
  - **Duplicate-dispatch batch (from the same-day partial_profit 4108 investigation, previously uncommitted):** `userListener.ts` claims message in-memory BEFORE parse (dropped on duplicate), releases claim on parse failure; `ensureSignalRow.ts` detects the unique-key owner → persists a **non-executable stub** (`status:'skipped'`, `skip_reason:'duplicate_telegram_message'`) and returns `{duplicate, existingSignalId}`; `dispatch.ts` `handleSignal` returns early on `ensured.duplicate` (worker-side defense); `managementExecutor.ts` partial volumes FLOOR + below-min-lot skip; `orderModifyBenign.ts` `isPositionGoneCloseError` (MT4 4108 benign for close-family only); `partialTpMonitor.ts` 4108 benign.
  - **NEW this session — parse retry:** `worker/src/withParseRetry.ts` (pure helper: bounded retries + backoff) wraps the listener parse call — transient parse failures (AI 429/timeout/transport) retry the SAME signal id (max 3, 750/1500ms backoff, env `LISTENER_PARSE_MAX_ATTEMPTS`), so a failed parse no longer loses the signal; final failure behavior unchanged (error row + claim release).
  - **Tests:** `withParseRetry.test.ts` (new, 6 cases), `ensureSignalRow.test.ts` + incident-shaped regression (close msg 19742 → owner executable, loser stub `skipped`; now 8 tests), existing batch tests (orderModifyBenign, partialTpMonitor, parseConfig, universalSignalParser). Targeted suites 55/55; worker `tsc --noEmit` clean; eslint clean (fixed one pre-existing unused-var in `universalSignalParser.ts`).
- **Design notes (user-driven):** "close after the error surfaces once" was already the deployed Aug-11 behavior (trades closed ~1.3s after first reject) — the remaining gap was the duplicate itself; the wait-and-take-over idea was replaced by the simpler, idempotent same-id parse retry.
- **Follow-ups (deployer):** commit batch (split: duplicate-dispatch / 4108+volume / parse-retry / Cerebras pool / docs) → PR to upstream/dev → staging (Railway listener + trade worker redeploy; let old listener instance fully stop first) → verify no duplicate mgmt rows + stubs land `skipped` → main. Historical prod rows stay as-is. PR #102 (deferred-only range plan) already on staging/dev; Emma's work (PR #94 etc.) verified present on all branches.
...
- **Root cause — FOUR stacked bugs (verified via prod DB + Railway):**
  ...
  2. **Executable stub (enabler):** `ensureSignalRow` hit the `signals_user_channel_telegram_message_unique_idx` conflict on the 2nd persist and fell back to a **telegram_message_id=NULL stub with status 'parsed'** — still dispatched + executed. Both signals ran `partial_profit` on ticket 421496628 concurrently: one orderClose succeeded (closed the position), the other got **4108 (ticket already closed)** → logged failed → Major.
  3. **Volume rounding (user-harm):** `lots = +(0.01 * 0.5).toFixed(2)` = `0.005 → "0.01"` — "book 50%" on a 0.01-lot position rounded UP to the FULL position. Even the "successful" execution closed 100% (trade row now lot_size 0.0, closed) instead of booking half and holding.
  4. **4108 not benign (alerting):** `isUnknownTicketError` / `isPositionGoneError` didn't match `4108` / `invalid request`, so the gone-position close surfaced as Major instead of benign.
- **Fixes (worker only, branch `feature/fix-mgmt-partial-profit-4108` from upstream/dev, NOT yet committed/pushed):**
  - `worker/src/userListener.ts`: `liveMessageDedup.set()` moved BEFORE the parse (right after the dedup check) so a concurrent 2nd delivery within 120s is dropped at the root; claim released on parse failure. Also: when `ensureSignalRow` returns `duplicate`, log + bumpLastSeen + return true WITHOUT dispatching.
  - `worker/src/ensureSignalRow.ts`: on telegram-message unique conflict, look up the owning signal; if found, persist a NON-executable stub (`status:'skipped'`, `skip_reason:'duplicate_telegram_message'`) and return `{ok:true, duplicate:true, existingSignalId}`. Sweep can't pick it up (filters `status='parsed'`); FK-recovery callers still get a row.
  - `worker/src/tradeExecutor/dispatch.ts`: `handleSignal` returns early when `ensured.duplicate` (defense in depth on the worker side).
  - `worker/src/tradeExecutor/managementExecutor.ts`: partial_profit + partial_breakeven volumes now FLOOR (`Math.floor(lot*frac*100)/100`) instead of `toFixed(2)`; if lots < 0.01 (min lot) the close is skipped with `status:'skipped'` mgmt log + `skip_reason:'partial_volume_below_min_lot'` (frontend already renders mgmt skipped rows), trade row untouched. Catch site: `positionGone` now also true for close-family actions via `isPositionGoneCloseError`.
  - `worker/src/orderModifyBenign.ts`: new exported `isPositionGoneCloseError()` — gone-position patterns + `4108`/`invalid request`, scoped to CLOSE-family only (NOT the shared `isPositionGoneError` used by modify paths, where 4108 can mean bad params, not a gone position).
  - `worker/src/partialTpMonitor.ts`: benign regex also matches `4108` / `invalid request` (same incident family).
- **Tests:** `ensureSignalRow.test.ts` (+duplicate-stub case incl. chainable select mock), `orderModifyBenign.test.ts` (+isPositionGoneCloseError), `partialTpMonitor.test.ts` (+4108). 76/76 worker tests pass; worker `tsc --noEmit` clean; eslint clean on all touched files. Frontend `channelWorkerLogMessage` + `tradeNotifications` tests 44/44 pass (skipped-mgmt rendering path already existed).
- **Also noted:** pre-existing lint errors (`_table` unused params) in `ensureSignalRow.test.ts` existed at HEAD; cleaned up while touching the file.
- **Follow-ups (deployer):** commit branch → push origin → PR to upstream/dev → staging (Railway redeploy) → main after validation. Historical prod rows stay as-is (backfill optional). The duplicate-delivery source (live event + poll overlap) is now harmless: worst case a 2nd signal is created but never dispatched.
- **Notion:** new task created/updated on board.
...
- **Implementation:**
  ...
  - `worker/src/signalIntent/universalSignalParser.ts`: `callCerebrasUniversal` now iterates the whole key pool instead of one key. Each call **always rotates the starting key** (`cerebrasKeyCursor = (startIndex + 1) % keys.length`, success or failure) → every message leads with a different account, load spreads evenly. `callChatCompletions` detects the daily-quota 429 via `isCerebrasDailyLimit` (`status===429 && /per day limit/i`) — no retry on that (quota can't recover mid-process); the key is marked exhausted in an in-memory `Map` and skipped for the process lifetime. Other 429s/5xx keep the existing per-key retry-with-backoff, then the next key is tried. When every key is exhausted the error says so explicitly before OpenAI fallback. Stage-2 gate updated to `cerebrasParseApiKeys().length > 0`.
  - Exported pure helpers `isCerebrasDailyLimit` + `cerebrasKeyOrder` (rotation + exhausted-skip ordering) for tests.
- **Tests:** `parseConfig.test.ts` +4 (legacy fallback, numbered collection in order, first-gap stop, empty), `universalSignalParser.test.ts` +8 (daily-limit detection incl. requests-per-day, transient 429/500 not flagged, rotation start advance, exhausted skip, all-exhausted, empty pool, out-of-range cursor). 22/22 pass via `node --require ts-node/register --test` on the two files.
- **Trade-offs:** in-memory exhaustion state resets on worker restart (re-learns per process; a key exhausted pre-restart is tried once before being skipped again); cursor is per-process so multi-replica listener shards each rotate independently (still balanced per shard); `CEREBRAS_API_KEY` untouched = zero-config backward compat.
- **Deploy steps (listener workers, staging then prod):** set `CEREBRAS_API_KEY_1=<key1>`, `CEREBRAS_API_KEY_2=<key2>`, … (one var per account; leave `CEREBRAS_API_KEY` as-is or unset), redeploy. No other code/env changes.
- **Notion:** task board reviewed 2026-08-12; no pre-existing task for this (board query tool call was interrupted mid-session; no matching task found in prior reviews).
