...
5. Ask “Open the broker configuration page” → if multiple brokers, asks which one; then opens `/brokers` + that broker’s config modal (never signup)
6. Ask “Can you run a backtest?” / “Open the backtest page” → opens `/backtest` with short steps (does **not** claim backtests are unsupported)
7. Ask “how did my last backtest do?” → `list_backtests` summary
8. Ask to pause the copier → Confirm → header pause state updates
9. Attach or paste a screenshot and ask what it shows
