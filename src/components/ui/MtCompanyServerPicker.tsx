...
export function MtCompanyServerPicker({
  ...
  const notifyServerChange = useCallback((next: string) => {
  ...
  const handleManualServerChange = (next: string) => {
    setManualServer(next)
    notifyServerChange(next)
  }
      ...
      {manualMode ? (
        <div className="space-y-2">
          <input
            ...
            value={manualServer}
            required={required}
            onChange={e => handleManualServerChange(e.target.value)}
