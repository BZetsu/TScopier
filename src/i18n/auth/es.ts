import { testimonialsEs } from '../testimonials/es'
import type { AuthTranslations } from './types'

export const authEs: AuthTranslations = {
  nav: {
    signIn: 'Iniciar sesión',
    createAccount: 'Crear cuenta',
    mobileTagline: 'Un copiador fluido para cada señal de Telegram',
    backHome: 'Volver al inicio',
  },
  oauth: {
    continueWithGoogle: 'Continuar con Google',
    orDivider: 'o',
    captchaRequired: 'Completa la verificación de seguridad antes de continuar.',
  },
  login: {
    heading: 'Inicia sesión en TScopier',
    noAccount: '¿No tienes cuenta?',
    signUpLink: 'Regístrate',
    footerPrompt: '¿Nuevo en TScopier?',
    footerLink: 'Crea una cuenta gratis',
    email: 'Correo electrónico',
    emailPlaceholder: 'tu@ejemplo.com',
    password: 'Contraseña',
    passwordPlaceholder: 'Introduce tu contraseña',
    forgotPassword: '¿Olvidaste tu contraseña?',
    passwordResetSuccess: 'Tu contraseña se actualizó. Ya puedes iniciar sesión.',
    submit: 'Iniciar sesión',
  },
  forgotPassword: {
    heading: 'Restablecer contraseña',
    subtitle:
      'Introduce el correo de tu cuenta. Si existe, te enviaremos un enlace para restablecer la contraseña.',
    email: 'Correo electrónico',
    emailPlaceholder: 'tu@ejemplo.com',
    submit: 'Enviar enlace',
    sentHeading: 'Revisa tu correo',
    sentSubtitle:
      'Si existe una cuenta para {email}, recibirás un enlace para restablecer la contraseña en breve.',
    sentHint: 'Revisa la carpeta de spam si no lo ves en unos minutos.',
    sendError: 'No se pudo enviar el correo de restablecimiento. Inténtalo de nuevo en un momento.',
    backToLogin: 'Volver al inicio de sesión',
  },
  resetPassword: {
    heading: 'Establecer una nueva contraseña',
    subtitle: 'Elige una contraseña segura para tu cuenta de TScopier.',
    verifyingHeading: 'Verificando tu enlace de restablecimiento…',
    verifyingSubtitle: 'Espera mientras validamos tu enlace de forma segura.',
    securityNote: 'Después de actualizar, cerrarás sesión en todos los dispositivos. Vuelve a iniciar sesión con tu nueva contraseña.',
    password: 'Nueva contraseña',
    passwordPlaceholder: 'Introduce una nueva contraseña',
    confirmPassword: 'Confirmar contraseña',
    confirmPasswordPlaceholder: 'Vuelve a escribir la contraseña',
    passwordHint: 'Al menos 6 caracteres con mayúsculas, minúsculas, un número y un símbolo',
    passwordTooShort: 'La contraseña debe tener al menos 6 caracteres',
    passwordTooWeak: 'La contraseña es demasiado débil. Usa mayúsculas, minúsculas, un número y un símbolo.',
    passwordMismatch: 'Las contraseñas no coinciden',
    submit: 'Actualizar contraseña',
    successHeading: 'Contraseña actualizada',
    successSubtitle: 'Tu nueva contraseña está lista. Redirigiendo al inicio de sesión…',
    invalidHeading: 'Enlace caducado o no válido',
    invalidSubtitle: 'Este enlace ya no es válido. Solicita uno nuevo para continuar.',
    requestNewLink: 'Solicitar nuevo enlace',
    backToLogin: 'Volver al inicio de sesión',
  },
  signup: {
    heading: 'Crea tu cuenta',
    hasAccount: '¿Ya tienes cuenta?',
    signInLink: 'Iniciar sesión',
    footerPrompt: '¿Ya tienes cuenta?',
    footerLink: 'Iniciar sesión',
    firstName: 'Nombre',
    firstNamePlaceholder: 'Nombre',
    lastName: 'Apellido',
    lastNamePlaceholder: 'Apellido',
    email: 'Correo electrónico',
    emailPlaceholder: 'tu@ejemplo.com',
    password: 'Contraseña',
    passwordPlaceholder: 'Elige una contraseña',
    confirmPassword: 'Confirmar contraseña',
    confirmPasswordPlaceholder: 'Vuelve a escribir tu contraseña',
    passwordHint: 'Al menos 6 caracteres con mayúsculas, minúsculas, un número y un símbolo',
    passwordTooShort: 'La contraseña debe tener al menos 6 caracteres',
    passwordTooWeak: 'La contraseña es demasiado débil. Usa mayúsculas, minúsculas, un número y un símbolo.',
    passwordMismatch: 'Las contraseñas no coinciden',
    emailNotAllowed: 'Esta dirección de correo no está permitida',
    disposableEmailNotAllowed: 'No se permiten direcciones de correo desechables',
    submit: 'Crear cuenta',
    terms: {
      prefix: 'Al registrarte, aceptas nuestros',
      termsOfService: 'Términos de servicio',
      conjunction: ' y la ',
      privacyPolicy: 'Política de privacidad',
    },
  },
  verify: {
    heading: 'Revisa tu correo',
    subtitle: 'Acabamos de enviar un enlace de verificación a {email}.',
    resend: 'Reenviar correo',
    resendIn: 'Reenviar en {seconds}s',
    resendCooldown: 'Espera {seconds}s antes de solicitar otro correo.',
    resent: 'Correo enviado!',
    backToLogin: 'Volver al inicio de sesión',
    confirmLinkExpired: 'Este enlace de confirmación expiró o ya fue usado. Inicia sesión o solicita un nuevo correo.',
  },
  welcome: {
    title: 'Bienvenido a TScopier',
    subtitle: 'Tu cuenta está lista. Conecta Telegram y empieza a copiar señales a tu bróker.',
    steps: [
      'Conecta Telegram y elige canales de señales',
      'Vincula tu cuenta MT4/MT5',
      'Copia operaciones automáticamente con tu configuración de riesgo',
    ],
    startUsing: 'Empezar a usar TScopier',
    exploreDashboard: 'Explorar el panel primero',
    errorFallback: 'No se pudo continuar. Inténtalo de nuevo.',
  },
  marketing: {
    headline: 'Un copiador fluido para cada señal de Telegram',
    trustpilotLabel: 'Trustpilot',
    reviews: testimonialsEs,
    copyright: '© {year} Tartarix Inc.',
  },
  language: {
    label: 'Idioma',
    choose: 'Elegir idioma',
    searchPlaceholder: 'Buscar idiomas…',
    noResults: 'No se encontraron idiomas',
  },
  theme: {
    light: 'Modo claro',
    dark: 'Modo oscuro',
    switchToLight: 'Cambiar a modo claro',
    switchToDark: 'Cambiar a modo oscuro',
  },
}
