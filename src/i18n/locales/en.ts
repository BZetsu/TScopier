...
export const en: Translations = {
  ...
  brokersPage: {
    title: 'FxSocket Brokers (MT5)',
    subtitle: 'Connect and test FxSocket cloud MT5 terminals side-by-side with your legacy mt4api.dev brokers.',
    mt5OnlyNote: 'FxSocket supports MT5 only. Legacy compare may be MT4 — quote and summary still work; demo trade compare is skipped for MT4.',
    apiKeyRotationNote: 'After rotating your FxSocket API key, allow ~5 minutes for terminals to pick up the new key. Brief 5xx responses during restart are normal.',
    setupNote: 'Link MT5 accounts with login, password, and server — only the FXSOCKET_API_KEY Edge secret is required (POST /v1/accounts). See the FxSocket v1 API docs for account linking; trading uses /mt5/{account_id}/…. Remove any old FXSOCKET_CONNECT_URL secret.',
    sectionAccounts: 'FxSocket accounts',
    sectionCompare: 'Side-by-side comparison',
    sectionRawLog: 'Raw API log',
    connectTitle: 'Connect MT5 via FxSocket',
    labelField: 'Label',
    ...
    noAccounts: 'No FxSocket accounts yet. Connect a demo account to start testing.',
    compareFxAccount: 'FxSocket account',
    ...
    demoTradeBody: 'This sends a micro-lot market buy on both FxSocket and the selected legacy demo broker (MT5 only). Only proceed on demo accounts you control.',
    ...
    loadError: 'Could not load FxSocket accounts.',
    connectError: 'Connect failed.',
    deleteConfirm: 'Remove this FxSocket account from TSCopier?',
    selectFxAccount: 'Select FxSocket account',
    ...
    docsEndpointNote: 'Trading and data calls use https://api.fxsocket.com/mt5/{account_id}/… as listed in the Request Builder (AccountSummary, getQuote, OrderSend, etc.). Requests run server-side via Edge — the API has no CORS headers.',
    linkExistingTitle: 'Advanced: link an account already connected in FxSocket',
    existingUuidField: 'Account UUID (from FxSocket Accounts page)',
  ...
  pages: {
    ...
    brokers: {
      title: 'Brokers',
      description: 'FxSocket MT5 sandbox — connect, diagnose, and compare against legacy mt4api.dev.',
    },
    contactSupport: {
